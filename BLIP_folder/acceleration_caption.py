...
class BLIP_Decoder(nn.Module):
    ...
    def generate(self, image, sample=False, num_beams=3, max_length=100, min_length=50, top_p=0.9, repetition_penalty=1.0):
        image_embeds = self.visual_encoder(image)
...
def main(args, config, r_value=40, alpha=1):   
    ...
    if r_value!=0:  
        if alpha!=0:
            folder.patch.folder(model1.visual_encoder)
            model1.visual_encoder.r=r_value
            model1.visual_encoder.alpha=alpha
            model1.visual_encoder.is_turbo=True
        else:
            folder.patch.folder_tome(model1.visual_encoder)
            model1.visual_encoder.r=r_value
